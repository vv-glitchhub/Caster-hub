    ...
    <section className="home-section">
      <div className="home-container">
        ...
        <p className="home-section-lead">
          Tiedot tallennetaan vain tämän laitteen localStorageen. Voit varmuuskopioida kaiken Account Data -sivulta ennen pilvisynkronoinnin käyttöönottoa.
        </p>
        ...
        <div className="home-module-card min-h-0">
          ...
          <div className="form-stack">
            <input className="caster-input" value={profile.nickname} onChange={(event) => updateProfile('nickname', event.target.value)} placeholder="Auton kutsumanimi" aria-label="Auton kutsumanimi" />
            <div className="home-highlight-grid form-summary-grid">
              <input className="caster-input" value={profile.make} onChange={(event) => updateProfile('make', event.target.value)} placeholder="Merkki" aria-label="Auton merkki" />
              <input className="caster-input" value={profile.model} onChange={(event) => updateProfile('model', event.target.value)} placeholder="Malli" aria-label="Auton malli" />
              <input className="caster-input" type="number" min="1886" max="2100" value={profile.year || ''} onChange={(event) => updateProfile('year', positiveNumber(event.target.value))} placeholder="Vuosimalli" aria-label="Auton vuosimalli" />
              <input className="caster-input" type="number" min="0" value={profile.mileageKm || ''} onChange={(event) => updateProfile('mileageKm', positiveNumber(event.target.value))} placeholder="Mittarilukema km" aria-label="Auton mittarilukema kilometreinä" />
              <input className="caster-input" value={profile.engine} onChange={(event) => updateProfile('engine', event.target.value)} placeholder="Moottori" aria-label="Auton moottori" />
              <input className="caster-input" value={profile.fuelType} onChange={(event) => updateProfile('fuelType', event.target.value)} placeholder="Polttoaine" aria-label="Auton polttoaine" />
              <input className="caster-input" value={profile.transmission} onChange={(event) => updateProfile('transmission', event.target.value)} placeholder="Vaihteisto" aria-label="Auton vaihteisto" />
              <input className="caster-input" value={profile.registration} onChange={(event) => updateProfile('registration', event.target.value.toUpperCase())} placeholder="Rekisteritunnus" aria-label="Auton rekisteritunnus" />
            </div>
          </div>
        </div>
        ...
        <div className="home-module-grid">
          <div className="home-module-card">
            ...
            <div className="form-stack">
              <input className="caster-input" value={faultCode} onChange={(event) => setFaultCode(event.target.value)} placeholder="P2177" aria-label="Vikakoodi" />
              <textarea className="caster-input" value={symptom} onChange={(event) => setSymptom(event.target.value)} placeholder="Kirjoita oire tähän" aria-label="Vian oire" rows={4} />
          ...
          <div className="home-module-card">
            ...
            <div className="form-stack">
              <input className="caster-input" value={serviceTitle} onChange={(event) => setServiceTitle(event.target.value)} placeholder="Esim. diagnostiikka" aria-label="Huollon nimi" />
              <input className="caster-input" value={serviceCost} onChange={(event) => setServiceCost(event.target.value)} placeholder="Hinta €" aria-label="Huollon arvioitu hinta" inputMode="decimal" />
          ...
          {services.map((service) => (
            <div key={service.id} className="home-module-card min-h-0">
              ...
              <div className="home-actions">
                <button className="secondary-button" type="button" onClick={() => toggleService(service.id)}>Mark {service.status === 'done' ? 'planned' : 'done'}</button>
