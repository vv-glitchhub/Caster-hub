...
export function buildProductionReadiness() {
  ...
  const migrationChecks: ReadinessCheck[] = [
    {
      ...
      detail: 'Carcaster and Travelcaster cloud tables must exist with row-level security.',
      nextAction: 'Apply supabase/caster_cloud_domains_v1.sql and verify ownership policies with two separate users.',
    ...
    },
    {
      ...
      detail: 'The authenticated `/api/account/cloud` route now provides RLS-scoped export and confirmation-gated account deletion code.',
      nextAction: 'Test cloud export and full cascading deletion with two production-like users, then set CASTER_ACCOUNT_LIFECYCLE_VERIFIED=true.',
      environmentVariable: 'CASTER_ACCOUNT_LIFECYCLE_VERIFIED',
    },
    {
      id: 'account-deletion-feature-flag',
      area: 'Privacy',
      title: 'Cloud account deletion explicitly enabled',
      status: process.env.CASTER_ACCOUNT_DELETION_ENABLED === 'true' ? 'ready' : 'blocked',
      detail: 'Permanent account deletion stays fail-closed behind an explicit production feature flag.',
      nextAction: 'Enable CASTER_ACCOUNT_DELETION_ENABLED=true only after lifecycle, storage ownership and two-user regression tests pass.',
      environmentVariable: 'CASTER_ACCOUNT_DELETION_ENABLED',
    },
    {
      id: 'auth-redirects',
      area: 'Identity',
  ...
  return {
    ...
    guardrails: [
      ...
      'Permanent account deletion requires a valid user JWT, exact confirmation phrase, matching email and an enabled production feature flag.',
