...
import { getFinlandAreaRankingV2 } from '../../../../lib/landcaster-v2'
...
export async function GET(request: NextRequest) {
  ...
  try {
    const result = await getFinlandAreaRankingV2(propertyType, Number.isFinite(limit) ? limit : 12)
    return NextResponse.json(result, { headers: { 'Cache-Control': 'public, s-maxage=21600, stale-while-revalidate=86400' } })
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Area Score V2 -vertailu epäonnistui'
